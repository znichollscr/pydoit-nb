"""
Plotting notebook-based step definition
...
from pydoit_nb.config_handling import get_config_for_step_id, get_step_config_ids
...
def _configure_notebooks(
    ...
    configs_make_draws = [
        get_config_for_step_id(config=config, step="make_draws", step_config_id=id)
        for id in get_step_config_ids(config.make_draws)
    ]
    configs_retrieve_data = [
        get_config_for_step_id(config=config, step="retrieve_data", step_config_id=id)
        for id in get_step_config_ids(config.retrieve_data)
    ]
    ...
    configured_notebooks = [
        ConfiguredNotebook(
            unconfigured_notebook=uc_nbs_dict[Path("110_make-plots")],
            configuration=(config_step.colour,),
            dependencies=(
                *(c.file_draws_scaled for c in configs_make_draws),
                *(c.file_clean_data for c in configs_retrieve_data),
            ),
            targets=(config_step.file_plot,),
            **common,
        ),
    ]
...
step = UnconfiguredNotebookBasedStep(
    ...
    unconfigured_notebooks=[
        UnconfiguredNotebook(
            notebook_path=Path("110_make-plots"),
            raw_notebook_ext=".py",
            summary="Make our plots",
            doc="Make our plots in a specific colour",
        ),
    ],
